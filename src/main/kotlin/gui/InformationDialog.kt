    ...
    private fun buildInformationDialog(phonebook: ContactService, columnButton: MutableList<JButton>, i: Int) {
        ...
        if (phonebook.getPersonDateOfBirth(phonebook.getAllPeople()[i]).isNotEmpty()) {
            labelDateOfBirth.text =
                labelDateOfBirth.text + phonebook.getPersonDateOfBirth(phonebook.getAllPeople()[i])[0].toString()
        } else {
        ...
        if (phonebook.getPersonPhones(phonebook.getAllPeople()[i]).size > 1) {
            labelOtherNumber.text =
                labelOtherNumber.text + phonebook.getPersonPhones(phonebook.getAllPeople()[i])[1].toString()
        } else {
        ...
        if (phonebook.getPersonEmails(phonebook.getAllPeople()[i]).size > 1) {
            labelOtherEmail.text =
                labelOtherEmail.text + phonebook.getPersonEmails(phonebook.getAllPeople()[i])[1].toString()
        } else {
        ...
        if (phonebook.getPersonAddress(phonebook.getAllPeople()[i]).isNotEmpty()) {
            labelAddress.text =
                labelAddress.text + phonebook.getPersonAddress(phonebook.getAllPeople()[i])[0].toString()
        } else {
